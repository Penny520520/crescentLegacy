export const nav = [
  {
    text: "about",
    path: "/about",
  },
  {
    text: "Legacy Collections",
    path: "/legacy",
  },
  {
    text: "Mixed-Use Developments",
    path: "/mixed-use",
  },
  {
    text: "Homeowner Care",
    path: "/homeowner-care",
  },
  {
    text: "contact",
    path: "/contact",
  },
];

export const legacyProjects = [
  {
    path: "../images/home/legacy-imgs/1.jpg",
    projectName: "Ascot",
    location: "Vancouver | West Side",
    status: "Now Selling",
    route: "https://crescentlegacy.com/ascot/",
    newTab: "1",
  },
  {
    path: "../images/home/legacy-imgs/2.jpg",
    projectName: "Linden House",
    location: "Vancouver | West Side",
    status: "Coming Soon",
    route: "/contact",
    newTab: "0",
  },
  {
    path: "../images/home/legacy-imgs/3.jpg",
    projectName: "New Brighton Homes",
    location: "Vancouver | East Side",
    status: "Coming Soon",
    route: "/contact",
    newTab: "0",
  },
  {
    path: "../images/home/legacy-imgs/6.jpg",
    projectName: "Paton Homes",
    location: "Vancouver | West Side",
    status: "Coming Soon",
    route: "/contact",
    newTab: "0",
  },
  {
    path: "../images/home/legacy-imgs/5.jpg",
    projectName: "Nanton House",
    location: "Vancouver | West Side",
    status: "Coming Soon",
    route: "https://crescentlegacy.com/ascot/",
    newTab: "1",
  },
  {
    path: "../images/home/legacy-imgs/4.jpg",
    projectName: "Laburnum House",
    location: "Vancouver | West Side",
    status: "Coming Soon",
    route: "/contact",
    newTab: "0",
  },
];

export const locationImgs = [
  {
    path: "../images/linden-house/location/1.jpg",
  },
  {
    path: "../images/linden-house/location/2.jpg",
  },
  {
    path: "../images/linden-house/location/3.jpg",
  },
  {
    path: "../images/linden-house/location/4.jpg",
  },
  {
    path: "../images/linden-house/location/5.jpg",
  },
  {
    path: "../images/linden-house/location/6.jpg",
  },
  {
    path: "../images/linden-house/location/7.jpg",
  },
  {
    path: "../images/linden-house/location/8.jpg",
  },
];
export const exteriorImgs = [
  {
    path: "../images/linden-house/exterior/1.jpg",
    caption: ["Courtyard / Unit E & F with Private Garden"],
  },
  {
    path: "../images/linden-house/exterior/2.jpg",
    caption: ["Architecture (Maple Cres.)"],
  },
  {
    path: "../images/linden-house/exterior/3.jpg",
    caption: ["Private Rooftop Terrace (Unit A / B / C / D)"],
  },
  {
    path: "../images/linden-house/exterior/1.jpg",
    caption: ["Courtyard / Unit E & F with Private Garden"],
  },
  {
    path: "../images/linden-house/exterior/2.jpg",
    caption: ["Architecture (Maple Cres.)"],
  },
  {
    path: "../images/linden-house/exterior/3.jpg",
    caption: ["Private Rooftop Terrace (Unit A / B / C / D)"],
  },
];
export const interiorImgs = [
  {
    path: "../images/linden-house/interior/1.jpg",
    caption: ["Unit F - Living + Dining + Kitchen (Dark Scheme)"],
  },
  {
    path: "../images/linden-house/interior/2.jpg",
    caption: ["Unit B - Living Space"],
  },
  {
    path: "../images/linden-house/interior/3.jpg",
    caption: ["Unit B - Kitchen (Light Scheme)"],
  },
  {
    path: "../images/linden-house/interior/4.jpg",
    caption: ["Primary Bedroom with Balcony"],
  },
  {
    path: "../images/linden-house/interior/5.jpg",
    caption: ["Primary Ensuite Bathroom"],
  },
  {
    path: "../images/linden-house/interior/6.jpg",
    caption: ["In-Home Elevator (Optional Upgrade for Unit B & C)"],
  },
];
export const interiorImgsM = [
  {
    path: "../images/linden-house/interior/1.jpg",
    caption: ["Unit F - Living + Dining + Kitchen (Dark Scheme)"],
  },
  {
    path: "../images/linden-house/interior/2.jpg",
    caption: ["Unit B - Living Space"],
  },
  {
    path: "../images/linden-house/interior/3.jpg",
    caption: ["Unit B - Kitchen (Light Scheme)"],
  },
  {
    path: "../images/linden-house/interior/4.jpg",
    caption: ["Primary Bedroom with Balcony"],
  },
  {
    path: "../images/linden-house/interior/5.jpg",
    caption: ["Primary Ensuite Bathroom"],
  },
  {
    path: "../images/linden-house/interior/6.jpg",
    caption: ["In-Home Elevator", "(Optional Upgrade for Unit B & C)"],
  },
];

export const locationMap = [
  {
    name: "map",
    path: "../images/linden-house/location/map/Linden House_Map_default.png",
  },
  {
    name: "Parks",
    path: "../images/linden-house/location/map/Linden House_Map_Park.png",
  },
  {
    name: "Membership",
    path: "../images/linden-house/location/map/Linden House_Map_Membership.png",
  },
  {
    name: "education",
    path: "../images/linden-house/location/map/Linden House_Map_Education.png",
  },
  {
    name: "Dining",
    path: "../images/linden-house/location/map/Linden House_Map_Dining.png",
  },
  {
    name: "Retail",
    path: "../images/linden-house/location/map/Linden House_Map_Retail.png",
  },
];
